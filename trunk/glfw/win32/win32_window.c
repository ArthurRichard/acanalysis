...
static int createWindow( const _GLFWwndconfig *wndconfig,
                         const _GLFWfbconfig *fbconfig , unsigned int parenthwnd)
{
    ...
    else
    {
		...
		if(parenthwnd!=0)
		{
			_glfwWin.dwExStyle=WS_EX_APPWINDOW|WS_EX_TOPMOST;
			_glfwWin.dwStyle=WS_CHILD;
		}
		if(parenthwnd!=0)
    _glfwWin.window = CreateWindowEx( WS_EX_APPWINDOW|WS_EX_TOPMOST,    // Extended style
                                      ...
                                      (HWND)parenthwnd,                  // No parent window
                                      NULL,                  // No menu
...
{
	return _glfwPlatformOpenSubWindow(width,height,wndconfig,fbconfig,0);
}
int  _glfwPlatformOpenSubWindow( int width, int height, const _GLFWwndconfig *wndconfig, const _GLFWfbconfig *fbconfig , unsigned int parenthwnd)
{
    ...
    if( !createWindow( wndconfig, fbconfig , parenthwnd) )
    {
