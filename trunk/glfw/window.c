...
{
	return glfwOpenSubWindow(width,height,redbits,greenbits,bluebits,alphabits,depthbits,stencilbits,mode,0);
}
GLFWAPI int GLFWAPIENTRY glfwOpenSubWindow( int width, int height,
    int redbits, int greenbits, int bluebits, int alphabits,
    int depthbits, int stencilbits, int mode , unsigned int parenthwnd)
{
    ...
    if( !_glfwPlatformOpenSubWindow( width, height, &wndconfig, &fbconfig ,parenthwnd) )
    {
        glfwCloseWindow();
        return GL_FALSE;
    }
